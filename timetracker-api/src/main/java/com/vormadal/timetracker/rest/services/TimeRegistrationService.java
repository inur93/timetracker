...
public class TimeRegistrationService {
    ...
    @PUT
    @Path("{tid}")
    public TimeRegistration updateTimeRegistration(@PathParam("tid") String id, UpdateTimeRegistration timeRegistration) throws AuthorizationException, DatabaseException {
        User user = getAuthUser(request);
