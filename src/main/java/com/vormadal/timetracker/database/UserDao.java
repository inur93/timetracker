...
public class UserDao extends MongoBaseDao<User, ObjectId> implements IUserDao {
    ...
    public void incrementFailedAttempts(ObjectId id) {
        ds().update(query(id), updateOperation().inc(Fields.User.failedLoginAttempts));
    }

    public void resetFailedAttempts(ObjectId id) {
        ds().update(query(id), updateOperation().set(Fields.User.failedLoginAttempts, 0));
    }

    @Override
    public void lockUser(ObjectId id) {
        ds().update(query(id), updateOperation()
                .set(Fields.User.locked, true)
                .set(Fields.User.failedLoginAttempts, 0));
    }
